...
2. **Start developing** – navigate into your directory with `cd my-carbon-site`.
   ...
   Start it up using one of the following snippets. You can tell which command to use based on the lock file at the root of your project (`yarn.lock` for yarn and `package-lock.json` for npm). For yarn, type `yarn develop` for npm, you'll use `npm run develop`.
